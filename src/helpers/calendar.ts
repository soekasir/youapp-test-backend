...
export class Ramalan {
  ...
  /** format DD-MM-YYYY atau 13-11-1996 */
  static getHoroscope(birthday: string) {
    const format = 'DD-MM-YYYY';
    const year = birthday.split('-')[2];
    const mBirthday = moment(birthday, format);
    for (const h in Ramalan.horoscopeData) {
      const before = moment(Ramalan.horoscopeData[h][0] + '-' + year, format);
      const after = moment(Ramalan.horoscopeData[h][1] + '-' + year, format);
      if (mBirthday.isSameOrAfter(before) && mBirthday.isSameOrBefore(after)) {
        return h;
      }
    }
  }
  ...
  /** format DD-MM-YYYY atau 13-11-1996 */
  static getZodiac(birthday: string) {
    const format = 'YYYY MMMM DD';
    const mBirthday = moment(birthday, 'DD-MM-YYYYY');
    for (const z of Ramalan.zodiacData) {
      const before = moment(z[0], format);
      const after = moment(z[1], format);
      if (mBirthday.isSameOrAfter(before) && mBirthday.isSameOrBefore(after)) {
        return z[2];
      }
    }
